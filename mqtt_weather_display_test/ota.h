void otaSetup() {
    ...
    NoFUSSClient.onMessage([](nofuss_t code) {
        if (code == NOFUSS_START) {
            ...
            drawCover(ICON_OTA, NULL, "Aktualizace FW");
    ...
    ArduinoOTA.onProgress([](unsigned int progress, unsigned int total) {
        ...
        if (ota_percentage != percentage) {
            ota_percentage = percentage;
    ...
    ArduinoOTA.onError([](ota_error_t error) {
        ...
        //drawCover(ICON_REFRESH, "Aktualizace FW se", "NEZDARILA");
        delay(1000);
    });
